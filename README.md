...
\begin{table*}[!ht]
    \footnotesize
    \caption{\label{tab:relations_table} Summary of the EoS-insensitive relations investigated in this work based on an extensive ensemble of NS configurations and $70$ tabulated EoS models of cold, and ultradense nuclear matter.}
    \begin{ruledtabular}
        \begin{tabular}{c|c|c|c}
            %\rowcolor{gray!25} {\bf Relation} & {\bf Equation} & {\bf Max} \\
            % \rowcolor{gray!25}& & {\bf $\%$ Error}\\
            \multirow{2}{*}{ {\bf Universal Relation}} & \multirow{2}{*}{\bf Parameters and their respective ranges}& \multirow{2}{*} {\bf Equation} & \multirow{2}{*}{\bf Max $\%$ Error} \\
               &  &     \\ 
            %   &  & {\bf $\%$ Error}    \\ 
            \hline
            $e(C,\sigma)$  & $C\in[0.088, 0.308], \ \sigma \in [0.033,0.961]$ &Improved Fit Eq.(\ref{eq:e_c_sigma}) &$ 4.57 $   \\
            \hline
             $ g_{\mathrm{pole}}(C,\sigma)$& $C\in[ 0.088,0.309], \ \sigma \in [0.000,0.961]$ & Improved Fit Eq.(\ref{eq:gpole_c_sigma}) & $ 3.07$  \\
             \hline
            $\mathcal{R}(C,\sigma)$ & $C\in[ 0.088,0.309], \ \sigma \in [0.000,0.961]$ & New Fit Eq.(\ref{eq:R_c_sigma}) & $ 2.79 $  \\
            \hline
            $(d \log R(\mu) / d \theta)_{\mathrm{max}} (C,\sigma,\mathcal{R})$ & $C\in[0.088, 0.308], \ \sigma \in [0.033,0.961], \ \mathcal{R}\in[0.626,0.981]$ & New Fit Eq.(\ref{eq:dlogR_c_sigma_Rp_Re}) & $ 3.21 $  \\
            \hline
            $ g_{\mathrm{eq}}(C,\sigma,e)$ &  $C\in[ 0.088,0.309], \ \sigma \in [0.000,0.961], \ e\in[0.000,0.780]$ & New Fit Eq.(\ref{eq:g_eq_c_sigma_e})& $ 4.26 $ \\
            \hline
            
            \multirow{2}{*}{$\forall \mu \in [0,1]: \ R(\mu;R_{\mathrm{pole}}, R_{\mathrm{eq}},C,\sigma,e)$} & $ \ R_{\mathrm{pole}} \in [8.618,14.161] \ \mathrm{km}, \ R_{\mathrm{eq}} \in [9.683,19.413] \ \mathrm{km},$ & \multirow{2}{*}{New Fit Eq.(\ref{eq:R_mu_fit})}& \multirow{2}{*}{$ 0.25 $} \\
            %\hline
             &  $C\in[ 0.088,0.309], \ \sigma \in [0.000,0.961], \ e\in[0.000,0.780]$& & \\
            
            
            \hline
            \multirow{2}{*}{$\forall \mu \in [0,1]: g(\mu;g_{\mathrm{pole}}, g_{\mathrm{eq}},C,\sigma,e)$} & $g_{\mathrm{pole}}/g_0 \in[0.987,2.107], \ g_{\mathrm{eq}}/g_0 \in [0.069,1.000],$& \multirow{2}{*}{New Fit Eq.(\ref{eq:g_mu_fit})}& \multirow{2}{*}{$ 0.91 $} \\
            
            & $C \in[ 0.088,0.309], \ \sigma \in [0.000,0.961], \ e\in[0.000,0.780]$& & \\

            \hline
           
           \multirow{2}{*} {\bf Universal Relation $\forall \mu \in [0,1]$} & \multirow{2}{*}{\bf Parameters and their respective ranges}& \multirow{2}{*} {\bf Equation} & \multirow{2}{*}{\bf Max Residual} \\
           ...
           %\cellcolor[gray]{0.7}
           \hline   
           \multirow{3}{*}{$ \left(\frac{d \log R(\mu)}{d\theta} \right)\left((d \log R(\mu) / d \theta)_{\mathrm{max}},C,\sigma,\mathcal{R}\right)$} & \multirow{2}{*}{$(d \log R(\mu) / d \theta)_{\mathrm{max}} \in [0.019,0.503], \ C \in[ 0.088,0.308],$} & \multirow{3}{*}{New Fit Eq.(\ref{dlogR_reg_fit})}& \multirow{3}{*}{$  8.36 \times 10^{-3}$}   \\
           
            \multirow{2}{*}{}& \multirow{2}{*}{$\sigma \in [0.033,0.961], \ \mathcal{R}\in[0.626,0.981]$}& \multirow{2}{*}{} & \multirow{2}{*}{} \\
            
            & &  &  \\

           %\\
            %\left(\frac{d \log R}{d\theta}\right)_{\mathrm{max}}
           % d \log R / d \theta)_{\mathrm{max}}
           %$ \left(d \log R(\mu)/d\theta \right) \left(\mu; (d \log R / d \theta)_{\mathrm{max}},C,\sigma,\mathcal{R} \right)$  & &  \\
          %  & New Fit Eq.(\ref{dlogR_reg_fit})&  $8.36\times 10^{-3}$&
        \end{tabular}
    \end{ruledtabular}
